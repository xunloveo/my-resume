...
export default {
  methods: {
    goHome () {
      this.$router.push({ name: 'home', params: { playAudio: true } })
    },
    ...
    process (vars) {
      if (vars.points.length < vars.initParticles)
        for (var i = 0; i < 5; ++i) this.spawnParticle(vars)
      var p, d, t
      ...
      for (var i = -25; i <= 25; i += 1) {
        for (var j = -25; j <= 25; j += 1) {
          ...
          if (point.d != -1) {
            ...
            if (a > 0) {
              vars.ctx.fillStyle = this.colorString(
                this.interpolateColors(
                  this.rgbArray(d / 26 - vars.frameNo / 40),
                  [0, 128, 32],
    ...
    frame (vars) {
      if (vars === undefined) {
        ...
        vars.camZ = -14
        vars.pitch =
          this.elevation(vars.camX, vars.camZ, vars.camY) - Math.PI / 2
        vars.yaw = 0
        vars.cx = vars.canvas.width / 2
        vars.cy = vars.canvas.height / 2
        vars.bounding = 10
  ...
  beforeDestroy () {
    window.cancelAnimationFrame(this.animation) // 取消该次动画
    this.animation = null
  }
}
</script>
